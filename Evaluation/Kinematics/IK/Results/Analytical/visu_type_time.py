...
def main():
    """
    Description:
        A program to visualize the calculation time for each successfully achieved goal within trajectory 
        calculation using analytical method of inverse kinematics.

        The observation is tested on trajectories generated using a multi-axis trapezoidal profile.

        Note:
            The program for the generation of the time can be found here:
    ...
    data = File_IO.Load(f'{file_path}/Method_Analytical_IK_Time', 'txt', ',')
    
    label = [r'Time in seconds']; title = ['Observed Time in Each Successfully Achieved Target']
