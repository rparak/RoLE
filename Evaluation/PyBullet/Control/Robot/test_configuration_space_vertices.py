...
def main():
    """
    Description:
        A program to test the configuration space by checking the vertices within a defined cuboid area.
    """
    ...
    PyBullet_Robot_Cls = PyBullet.Robot.Core.Robot_Cls(Robot_Str, f'{CONST_PROJECT_FOLDER}/URDFs/Robots/{Robot_Str.Name}/{Robot_Str.Name}.urdf', 
    ...
    while PyBullet_Robot_Cls.is_connected == True:
        ...
        T_vertex = HTM_Cls(None, np.float64).Rotation(PyBullet_Robot_Cls.T_EE.Get_Rotation('QUATERNION').all(), 'QUATERNION').Translation(PyBullet_Robot_Cls.C[i])
