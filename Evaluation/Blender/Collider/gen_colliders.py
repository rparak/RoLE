...
def main():
    ...
    for _, obj in enumerate(bpy.data.objects):
        ...
        if 'Collision' in obj.name:
            Blender.Utilities.Remove_Object(obj.name)
            continue

        # Get positions of the vertices of the mesh object.
        obj_i_verts = Blender.Utilities.Get_Vertices_From_Object(obj.name)
        ...
        Blender.Utilities.Create_Primitive('Cube', obj_i_name_new, box_properties)
        ...
        Blender.Utilities.Set_Object_Origin(obj_i_name_new, HTM_Cls(None, np.float64).all())


        # Remove all materials from the created object.
        Blender.Utilities.Remove_Object_Material(obj_i_name_new)
