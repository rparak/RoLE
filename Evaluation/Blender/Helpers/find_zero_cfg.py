...
def main():
    ...
    i = 0
    while True:
        if Blender.Utilities.Object_Exist(f'Viewpoint_Joint_{i}') == True:
            Blender.Utilities.Remove_Object(f'Viewpoint_Joint_{i}')
        else:
            break     
        i += 1
    ...
    Blender.Utilities.Object_Visibility('Viewpoint', True)
    
    # Add viewpoints with the correct transformation to object joints.
    Blender.Utilities.Add_Viewpoints('Viewpoint', Robot_Str.T.Zero_Cfg)
