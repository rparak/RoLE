...
class Poly_3D_Cls(object):
    ...
    def __init__(self, name: str, curve_properties: tp.Tuple[float, tp.List[float]], point_properties: tp.Tuple[tp.Union[None, bool], float, tp.List[float]]) -> None: 
        ...
        RoLE.Blender.Utilities.Remove_Object(self.__name)
    ...
    def __Add_Sphere(self, index: int, x: float, y: float, z: float) -> None:
        ...
        RoLE.Blender.Utilities.Create_Primitive('Sphere', f'Point_{index}', point_properties)
...
class Mechanism_Cls(object):
    ...
    def __init__(self, Mechanism_Parameters_Str: RoLE.Parameters.Mechanism.Mechanism_Parameters_Str, visibility: tp.Dict[str, str]) -> None:
        try:
            assert RoLE.Blender.Utilities.Object_Exist(f'{Mechanism_Parameters_Str.Name}_ID_{Mechanism_Parameters_Str.Id:03}') == True
            ...
            if RoLE.Blender.Utilities.Object_Exist(f'Viewpoint_EE_{self.__name}'):
                RoLE.Blender.Utilities.Object_Visibility(f'Viewpoint_EE_{self.__name}', visibility['Viewpoint_EE'])
            ...
            for _, collider_name in enumerate(np.concatenate((list(self.__Mechanism_Parameters_Str.Collider.Base), 
                ...
                if RoLE.Blender.Utilities.Object_Exist(collider_name):
                    RoLE.Blender.Utilities.Object_Visibility(collider_name, visibility['Colliders'])
    ...
    def Theta(self) -> float:
        ...
        ax_i_id_num = RoLE.Blender.Utilities.Convert_Ax_Str2Id(self.__Mechanism_Parameters_Str.Theta.Axis)
    ...
    def Add_External_Object(self, T: tp.List[tp.List[float]], color: tp.List[float], 
        ...
        RoLE.Blender.Utilities.Create_Primitive('Cube', object_name, box_properties)

        # Set the object transformation.
        RoLE.Blender.Utilities.Set_Object_Transformation(object_name, T)
    ...
    def Remove_All_External_Objects(self) -> None:
        ...
        while True:
            ...
            if RoLE.Blender.Utilities.Object_Exist(object_name) == True:
                RoLE.Blender.Utilities.Remove_Object(object_name)
            else:
                break     
            i += 1
            ...
            for _, (t_i, theta_arr_i) in enumerate(zip(self.__Trapezoidal_Cls.t, theta_arr)):
                ...
                if self.__Mechanism_Parameters_Str.Theta.Limit[0] <= theta_arr_i <= self.__Mechanism_Parameters_Str.Theta.Limit[1]:
                    ...
                    if self.__Mechanism_Parameters_Str.Theta.Type == 'R':
                        ...
                        RoLE.Blender.Utilities.Insert_Key_Frame(self.__Mechanism_Parameters_Str.Theta.Name, 'matrix_basis', np.int32(t_i * self.__fps), 'ALL')
                else:
                    # Update the scene.
                    self.__Update()
...
class Robot_Cls(object):
    ...
    def __init__(self, Robot_Parameters_Str: RoLE.Parameters.Robot.Robot_Parameters_Str, visibility: tp.Dict[str, str]) -> None:
        try:
            assert RoLE.Blender.Utilities.Object_Exist(f'{Robot_Parameters_Str.Name}_ID_{Robot_Parameters_Str.Id:03}') == True
            ...
            if RoLE.Blender.Utilities.Object_Exist(f'Workspace_{self.__name}'):
                RoLE.Blender.Utilities.Object_Visibility(f'Workspace_{self.__name}', visibility['Workspace'])
    ...
    def Theta(self) -> tp.List[float]:
        ...
        for i, (th_i_name, T_i_zero_cfg, ax_i, th_i_type) in enumerate(zip(self.__Robot_Parameters_Str.Theta.Name, T_zero_cfg, 
            ...
            # Convert a string axis letter to an identification number.
            ax_i_id_num = RoLE.Blender.Utilities.Convert_Ax_Str2Id(ax_i)
            ...
            for _, (t_i, theta_arr_i) in enumerate(zip(self.__Polynomial_Cls.t, np.array(theta_arr, dtype=np.float64).T)):
                ...
                for i, (th_i, th_i_name, T_i_zero_cfg, th_i_limit, ax_i, th_i_type, th_i_dir) in enumerate(zip(theta_arr_i, self.__Robot_Parameters_Str.Theta.Name, T_zero_cfg, 
                    ...
                    if th_i_limit[0] <= th_i <= th_i_limit[1]:
                        ...
                        # Insert a keyframe of the object (Joint_{i}) into the frame at time t(i). 
                        RoLE.Blender.Utilities.Insert_Key_Frame(th_i_name, 'matrix_basis', np.int32(t_i * self.__fps), 'ALL')
